...
## 5a. Guide for other admins

**https://gws.mcp.cheminneuf.community/admin** (FR/EN): for Claude Team Owners (add the connector: Organization settings → Connectors → Add → Custom → Web; Advanced settings empty; don't use Enterprise-managed auth) and Google Workspace admins (approve the client ID with *Specific* access), plus onboarding, the 100-user lifetime cap, access management, troubleshooting and who does what. Source: `cn_extras/static/admin/index.html`.
