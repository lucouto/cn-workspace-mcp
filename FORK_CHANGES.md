...
| 3 | `core/tool_tiers.yaml` | Add a `cn_extras` service block (or register the tools outside the tier system) | Tool filtering applies to our tools |
| 4 | `pyproject.toml` | Add `pypdfium2`, `Pillow`, `azure-ai-documentintelligence`, `azure-core`; add `cn_extras` to packages | Dependencies |
