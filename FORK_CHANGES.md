...
| 4 | `pyproject.toml` | Optional extra `cn` (Pillow now; `pypdfium2`, `azure-ai-documentintelligence` in Phase 2b). `cn_extras` is picked up by `packages.find` automatically. | Dependencies |
...
| 2026-09-23 | `pyproject.toml` | +4 lines: optional extra `cn = ["pillow>=11.0.0"]` | Image resizing in `gmail_read_attachment`. Deploy must install `--extra cn` (Dockerfile, Phase 4) |
