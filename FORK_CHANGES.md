...
| 2026-09-23 | (new files, no upstream edit) `Dockerfile.cn`, `deploy/coolify.env.example`, `docs/DEPLOY.md`, `docs/privacy-page.md`, `docs/admin-request-note.md`, `cn_extras/admin.py` | Phase 4. Upstream's `Dockerfile` / `docker-compose.yml` untouched. `cn_extras/admin.py` mirrors core/server.py's disk-store key derivation — keep in sync (verified against a real server's files) |
| 2026-09-23 | (new files) `cn_extras/privacy.py`, `cn_extras/static/privacy/` | Public `/privacy` page for Google's consent screen, via `server.custom_route` from cn_extras (no upstream edit). Exact-name file allowlist, strict CSP. Fonts are OFL substitutes (Jost, Cormorant Garamond, Playfair Display) with their licences; the licensed CCN fonts must not be published |
